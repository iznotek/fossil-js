...
* `fossil.cwd` can now be configured to affect just the chain rather than root instance. ([4110662](https://www.github.com/steveukx/git-js/commit/411066241c014c609d18a37e128c38f2c947c6e7))
...
* Export `FossilPluginError` from the main package. ([2aa7e55](https://www.github.com/steveukx/git-js/commit/2aa7e55216cdf57ca905cd6c23ff6b71002450c6)), closes [#616](https://www.github.com/steveukx/git-js/issues/616)
...
* Export missing `SimpleFossilProgressEvent` ([038870e](https://www.github.com/steveukx/git-js/commit/038870eb9ae35be78c1dd7fe1977ad8ba35913f2)), closes [#601](https://www.github.com/steveukx/git-js/issues/601)
...
* Support parsing `fossil.branch` where branches have carriage returns in the commit detail. ([5b71012](https://www.github.com/steveukx/git-js/commit/5b710125a5afde5fc1310c5a092cc7c48930c9bb))
...
* Update documentation for configuring `SimpleFossil` - `options` should be a `Partial<SimpleFossilOptions>` to allow for supplying just some of its properties. ([30523df](https://www.github.com/steveukx/git-js/commit/30523dff5bcd483b8fa778ae73caaa84057faad4)), closes [#580](https://www.github.com/steveukx/git-js/issues/580)
...
* refactor `git push` to TypeScript `SimpleFossilBase` interface ([e77ef1b](https://www.github.com/steveukx/git-js/commit/e77ef1b1adf89722571fca3f3547b5d8dfbc9d84))
* refactor `git push` to TypeScript `SimpleFossilBase` interface ([0691e85](https://www.github.com/steveukx/git-js/commit/0691e855124e2dc5fdb3403ada30afcd157047c4))
...
* split the `fossil.add` into the ts `SimpleFossilApi` ([14432f9](https://www.github.com/steveukx/git-js/commit/14432f9879744cafa043c0fbeee00b37db726f81))
...
- Supports passing configuration arguments to the `git` binary (via its `-c` argument as a prefix to any other
  ...
  `simpleFossil('/some/path', { config: ['http.proxy=someproxy'] }).pull()` 
- Add deprecation notice to `fossil.silent`
- Internal Updates:
...
## 2.29.0 Update TypeScript response type for `fossil.mergeFromTo`

- Update type definition for `fossil.mergeFromTo` to be the `MergeResult` returned
  when using the more generic `fossil.merge` method.
...
- Adds support for `fossil.applyPatch` to apply patches generated in a `git diff` to the working index,
...
-  Update the `fossil.status` parser to account for staged deleted/modified files and staged files with subsequent
...
## 2.26.0 Fix error when using `fossil.log` with callback

- Resolves an issue whereby using `fossil.log` with a callback (or awaiting the promise created from the now deprecated
  `simple-fossil/promise` import) would fail to return the response to the caller. 

## 2.25.0 TypeScript Types & Unit Tests, Commit Parsing

- See [Legacy Node Versions](./docs/LEGACY_NODE_VERSIONS.md) for details of how to use `simple-fossil` with `node.js`
...
- When using `fossil.commit`, the first argument must be a string or array of strings. Passing another data type has long
  been considered an error, but now a deprecation warning will be shown in the log and will be switched to an error
  in version 3.
- Fixes an issue in `fossil.commit` whereby a commit that included only deleted lines would be parsed as though the
...
## 2.22.0 add `fossil.hashObject` interface

- Adds support for `git hash-object FILE` and `git hash-object -w FILE`
  with new interface `fossil.hashObject(...)`, with thanks to [@MiOnim](https://github.com/MiOnim)

## 2.21.0 add `string[]` to `LogOptions` type

- Adds `string[]` to the set of types supported as options for `fossil.log`
- Fix readme typos 
...
- `fossil.clone` and `fossil.mirror` rewritten to fit the TypeScript tasks style.
- resolves issue whereby `fossil.clone` didn't accept an object of options despite being documented as supporting.
...
- `fossil.mv` rewritten to fit the TypeScript tasks style.
...
## 2.14.0 - Bug fix: `fossil.checkoutBranch` fails to pass commands to git child process

- resolves an issue whereby the `fossil.checkoutBranch` method would not pass the branch detail through to the underlying child process.
...
## 2.13.1 - Add support for parsing FossilLab Pull Request Url Message

- Further to `2.13.0` adding support for parsing the reponse to `fossil.push`, adds support for the pull request message
  used by gitlab.
...
## 2.12.0 - Bug fix: chaining onto / async awaiting `fossil.tags` failed

- resolves an issue whereby the `fossil.tags` method could not be chained or used as an async/promise.

## 2.11.0 - Parallel / concurrent tasks, fresh repo status parser & bug-fix in `checkoutLocalBranch`

- until now, `simple-fossil` reject all pending tasks in the queue when a task has failed. From `2.11.0`, only
  ...
  `simple-fossil` will limit the number of tasks running in parallel at any one time to be at most 1 from each
  chain (ie: chained tasks are still run in series) and at most 5 tasks across all chains (
  [configurable](./readme.md#configuration) by passing `{maxConcurrentProcesses: x}` in the `simpleFossil` constructor). 

- add support to `fossil.status()` for parsing the response of a repo that has no commits yet, previously
...
- `fossil.checkout` now supports both object and array forms of supplying trailing options.

```typescript
import simpleFossil from 'simple-fossil';
await simpleFossil().checkout('branch-name', ['--track', 'remote/branch']);
await simpleFossil().checkout(['branch-name', '--track', 'remote/branch']);
await simpleFossil().checkout({'branch-name': null});
```

- `fossil.init` now supports both object and array forms of supplying trailing options and now
  parses the response to return an [InitResult](./typings/response.d.ts);

```typescript
import simpleFossil, { InitResult } from 'simple-fossil';
const notSharedInit: InitResult = await simpleFossil().init(false, ['--shared=false']);
const notSharedBareInit: InitResult = await simpleFossil().init(['--bare', '--shared=false']);
const sharedInit: InitResult = await simpleFossil().init(false, {'--shared': 'true'});
const sharedBareInit: InitResult = await simpleFossil().init({'--bare': null, '--shared': 'false'});
```

- `fossil.status` now supports both object and array forms of supplying trailing options.

```typescript
import simpleFossil, { StatusResult } from 'simple-fossil';
const repoStatus: StatusResult = await simpleFossil().status();
const subDirStatus: StatusResult = await simpleFossil().status(['--', 'sub-dir']);
```

- `fossil.reset` upgraded to the new task style and exports an enum `ResetMode` with all supported
  merge modes and now supports both object and array forms of supplying trailing options.

```typescript
import simpleFossil, { ResetMode } from 'simple-fossil';

// git reset --hard
await simpleFossil().reset(ResetMode.HARD);

// git reset --soft -- sub-dir
await simpleFossil().reset(ResetMode.SOFT, ['--', 'sub-dir']);
```

- bug-fix: it should not be possible to await the `simpleFossil()` task runner, only the tasks it returns.

```typescript
expect(simpleFossil().then).toBeUndefined();
expect(simpleFossil().init().then).toBe(expect.any(Function));
```
...
- Enables support for using the default export of `simple-fossil` as an es module, in TypeScript it is no
  longer necessary to enable the `esModuleInterop` flag in the `tsconfig.json` to consume the default
  export.

### 2.7.2 - Bug Fix: Remove `promise.ts` source from `simple-fossil` published artifact
...
### 2.7.1 - Bug Fix: `await fossil.log` having imported from root `simple-fossil`

- Fixes #464, whereby using `await` on `fossil.log` without having supplied a callback would ignore the leading options
...
- All logging now uses the [debug](https://www.npmjs.com/package/debug) library. Enable logging by adding `simple-fossil`
  to the `DEBUG` environment variable. `fossil.silent(false)` can still be used to explicitly enable logging and is
  equivalent to calling `require('debug').enable('simple-fossil')`. 

## 2.6.0 - Native Promises, Typed Errors, TypeScript Importing, Fossil.clean and Fossil.raw

### Native Promises

- _TL;DR - `.then` and `.catch` can now be called on the standard `simpleFossil` chain to handle the promise
  returned by the most recently added task... essentially, promises now just work the way you would expect
  them to._
- The main export from `simple-fossil` no longer shows the deprecation notice for using the
...
- Promise / async interface and TypeScript types all available from the `simple-fossil` import rather than needing
  `simple-fossil/promise`, see examples in the [ReadMe](./readme.md) or in the [consumer tests](./test/consumer).
...
- Tasks that previously rejected with a custom object (currently only `fossil.merge` when the auto-merge fails)
  will now reject with a [`FossilResponseError`](./src/lib/errors/git-response-error.ts) where previously it
  was a modified `Error`.

### Fossil Clean

- `fossil.clean(...)` will now return a `CleanSummary` instead of the raw string data

### Fossil Raw

- `fossil.raw(...)` now accepts any number of leading string arguments as an alternative to the
  single array of strings.

## 2.5.0 - Fossil.remote

- all `fossil.remote` related functions converted to TypeScript

## 2.4.0 - Fossil.subModule

- all `fossil.subModule` related functions converted to TypeScript

## 2.3.0 - Fossil.config

- add new `fossil.listConfig` to get current configuration
- `fossil.addConfig` supports a new `append` flag to append the value into the config rather than overwrite existing

## 2.2.0 - Fossil.branch

- all `fossil.branch` related functions converted to TypeScript
- add new `fossil.deleteLocalBranches` to delete multiple branches in one call
- `fossil.deleteLocalBranches` and `fossil.deleteLocalBranch` now support an optional `forceDelete` flag
...
- `fossil.log` date format is now strict ISO by default (ie: uses the placeholder `%aI`) instead of the 1.x default of
